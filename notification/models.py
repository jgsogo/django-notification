...
def create_notice_type(label, display, description, default=2, verbosity=1, content_type=None):
    """
    ...
    try:
        ...
        if default != notice_type.default:
            ...
            updated = True
        if content_type != notice_type.content_type:
            notice_type.content_type = content_type
            update = True
        if updated:
            notice_type.save()
            if verbosity > 1:
        ...
        NoticeType(label=label, display=display, description=description, default=default, content_type=content_type).save()
        if verbosity > 1:
...
def send_now(users, label, object=None, extra_context=None, on_site=True, sender=None):
    """
    ...
    if object:
        ...
        notice_type_qs = notice_type_qs.filter(content_type=ctype)

    notice_type = notice_type_qs.get()
...
def queue(users, label, object=None, extra_context=None, on_site=True, sender=None):
    """
    ...
    notices = []
    for user in users:
        notices.append((user, label, object, extra_context, on_site, sender))
...
class ObservedItemManager(models.Manager):
    ...
    def all_for(self, observed, signal):
        ...
        observed_items = self.filter(notice_type__content_type=content_type, object_id=observed.id, signal=signal)
        return observed_items

    def get_for(self, observed, observer, signal):
        content_type = ContentType.objects.get_for_model(observed)
        observed_item = self.get(notice_type__content_type=content_type, object_id=observed.id, user=observer, signal=signal)
